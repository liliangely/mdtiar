...
yes :- write('Formula posiada dowod w systemie tabel analitycznych'), nl.
no :- write('Formula nie posiada dowodu w systemie tabel analitycznych'), nl.
...
*/
writename(X):-
  X=1->write('jeden');write('niejeden').


slebioda(X) :-
  expand([[neg X]], Y),
	closed(Y) -> write('Formula posiada dowod w systemie tabel analitycznych');write('Formula nie posiada dowodu w systemie tabel analitycznych').
